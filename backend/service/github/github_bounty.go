...
type BountyGithubI interface {
	CommentOnEvent(ctx context.Context, event github.IssueCommentEvent, msg string) error
	...
	GetCloseBountyMessage(ctx context.Context, event github.IssueCommentEvent, solverIdentities []identity.Identity) (string, error)
}
...
func (b BountyGithub) CommentOnEvent(ctx context.Context, event github.IssueCommentEvent, msg string) error {
	if _, _, err := b.client.Issues.CreateComment(ctx, event.GetRepo().GetOwner().GetLogin(), event.GetRepo().GetName(), event.GetIssue().GetNumber(), &github.IssueComment{
		Body: &msg,
	}); err != nil {
		return err
	}
...
func (b BountyGithub) GetCloseBountyMessage(ctx context.Context, event github.IssueCommentEvent, solverIdentities []identity.Identity) (string, error) {
	prettySolvers := ""
	for _, identity := range solverIdentities {
		prettySolvers += fmt.Sprintf("%s ", identity.Address)
	}
	msg := fmt.Sprintf("BBounty has been closed by %s ", prettySolvers)
	return msg, nil
}
